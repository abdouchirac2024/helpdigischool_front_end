...
export default function Features() {
  return (
    <>
      <Helmet>
        <title>Fonctionnalités - Help Digi School | Gestion Scolaire Complète</title>
        <meta 
          name="description" 
          content="Découvrez toutes les fonctionnalités de Help Digi School : notes, bulletins PDF, paiements Mobile Money, notifications SMS et plus encore." 
        />
      </Helmet>
      ...
      <div className="min-h-screen bg-background">
        ...
        <main className="pt-24">
          ...
          {featureCategories.map((category, i) => (
            <section 
              key={i} 
              className={`py-16 lg:py-24 ${i % 2 === 1 ? 'bg-muted/30' : ''}`}
            >
              <div className="container mx-auto px-4">
                <div className={`grid lg:grid-cols-2 gap-12 items-center ${i % 2 === 1 ? 'lg:flex-row-reverse' : ''}`}>
                  {/* Content */}
                  <div className={i % 2 === 1 ? 'lg:order-2' : ''}>
                    <div className={`w-14 h-14 rounded-xl bg-${category.color}/10 flex items-center justify-center mb-6`}>
                    ...
                    <div className="grid sm:grid-cols-2 gap-4">
                      {category.features.map((feature, j) => (
                        <div key={j} className="flex items-start gap-3 p-4 rounded-xl bg-card border border-border/50">
                          <div className={`w-10 h-10 rounded-lg bg-${category.color}/10 flex items-center justify-center shrink-0`}>
                            <feature.icon className={`w-5 h-5 text-${category.color}`} />
                          </div>
                          <div>
                  ...
                  <div className={`${i % 2 === 1 ? 'lg:order-1' : ''}`}>
                    <div className={`aspect-square rounded-3xl bg-gradient-to-br from-${category.color}/10 to-${category.color}/5 flex items-center justify-center p-12`}>
                      <category.icon className={`w-32 h-32 text-${category.color}/30`} />
                    </div>
                  </div>
                </div>
              </div>
